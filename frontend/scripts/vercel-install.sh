...
# Paquetes públicos → npmjs. @paqsuite/react-core → tarball vendored (misma 2.4.12 del lock).
set -euo pipefail

unset NPM_CONFIG_REGISTRY || true
unset npm_config_registry || true

tarball="vendor/paqsuite-react-core-2.4.12.tgz"
if [[ ! -f "${tarball}" ]]; then
...
pkg.dependencies['@paqsuite/react-core'] = 'file:vendor/paqsuite-react-core-2.4.12.tgz'
