...
test('consulta detallada muestra emitir y abre el dialogo', async ({ page }) => {
  ...
  await page.goto('/login?cliente=DEMO')
...
test('disenador lista proceso N=1 y monta DX solo tras confirmar', async ({ page }) => {
  ...
  await page.goto('/emisiones/disenador?processCode=partes.informes.consultaDetallada')
  ...
  await expect(page.getByTestId('emission.design.mount')).toHaveCount(0)

  await page.getByTestId('emission.design.confirmProcess').click()
  await expect(page.getByTestId('emission.design.mount')).toBeVisible({ timeout: 20_000 })
